...
public class FileLoader {
    ...
    public ArrayList<String[]> loadFile(String filePath) throws IOException {
        ...
        ArrayList<String[]> arrayOfRecords = new ArrayList<>(printNewCSV(records));
     ...
     */
    private ArrayList<String[]> printNewCSV(Iterable<CSVRecord> file) {
        double rainfallAmount = 0.0, min = Double.POSITIVE_INFINITY, max = 0.0;
        double recordRainfallAmountDouble;
        ArrayList<String[]> formattedRecords = new ArrayList<>();
        ...
        for (CSVRecord record : file) {      // loops through each data set
            ...
            String recordRainfallAmount = record.get("Rainfall");   // gets the usable data from CSV file
            ...
            if (recordDay == 1) {
                ...
                // this is a fix to a type error when trying to pass a string as a string[] through the graph
                // in the RainfallVisualizer, can be written better
                String[] recordsAsStringList = new String[5];
                recordsAsStringList[0] = recordYear;
                recordsAsStringList[1] = recordMonth;
                recordsAsStringList[2] = rainfallAmountString;
                recordsAsStringList[3] = "" + max;
                recordsAsStringList[4] = "" + min;

                formattedRecords.add(recordsAsStringList);

                rainfallAmount = 0.0; min = Double.POSITIVE_INFINITY; max = 0.0;    // resets values
            }
        }
